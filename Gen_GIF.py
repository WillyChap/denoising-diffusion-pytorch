...
##### Usage ######: 
#accelerate launch Gen_FIG.py --month "$MONTH" --co2 "$co2" --run_num 3944
#################
...
def main():
    ...
    fname = sorted(glob.glob(f'/glade/derecho/scratch/wchapman/Gen_CESM/Gen_GIF/samples_{do_run}_{run_num:03}_month{month_do:02}_co2{co2}*'))
   ...
   # Convert the scalar into a DataArray
    ...
    while accumulated_samples < target_total_samples:
        ...
        print('...starting to gather samples...')
        # Sample on each process
        all_images_list = diffusion.sample(batch_size=local_batch_size, x_cond=x_cond_rand[:local_batch_size, :, :, :], return_all_timesteps = True)
        print('...done gathering samples...')
    ...
    if trainer.accelerator.is_main_process:
        ...
        print('final samples shape', np.shape(final_samples))
        ...
        num_samples, numsteps, height, width = PS.shape  # Assume the same shape for all variables
        ...
        stepers = np.arange(numsteps)

        # Create the xarray Dataset
        ds = xr.Dataset(
            {
                'PS': (('samples', 'steps', 'lat', 'lon'), PS), 
                'PRECT': (('samples', 'steps', 'lat', 'lon'), PRECT), 
                'TREFHT': (('samples', 'steps', 'lat', 'lon'), TREFHT)
            },
            coords={
                'samples': samples,
                'steps': stepers,
        ...
        saved_successfully = save_file(f'/glade/derecho/scratch/wchapman/Gen_CESM/Gen_GIF/samples_{do_run}_{run_num:03}_month{month_do:02}_co2{co2}.nc', DS)
